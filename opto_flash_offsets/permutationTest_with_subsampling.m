...
function [p, observeddifference, effectsize] = permutationTest_with_subsampling(sample1, sample2, permutations, varargin)
%the original code by the authors performes a classical permutation test.
%To remove the bias when sample1>>sample2, the larger sample will always be
%subsampled to the matching size.
...
for n = 1:permutations
    ...
    else, permutation = randperm(equalsets_length); end
    ...
    allinds=[ind1,length(ind1)+ind2];
    % dividing into two samples
    randomSample1 = allobservations(allinds(permutation(1:nequal)));
    randomSample2 = allobservations(allinds(permutation(nequal+1:end)));
